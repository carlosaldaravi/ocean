  ...
  >
    <div class="w-full h-full p-4 xl:min-h-100">
      ...
      <div class="flex justify-between" v-if="editing">
        ...
        <div>
          <button @click="saveTarget()">
            <span>
              <svg class="inline-block w-8 h-8 p-1" viewBox="0 0 20 20" fill="green">
                <path
          ...
          >
            <span>
              <svg class="inline-block w-8 h-8 p-1" viewBox="0 0 20 20" fill="red">
                <path
      ...
      <div class="flex justify-between">
        ...
        >
          <span>
            <svg class="inline-block w-8 h-8 p-1" viewBox="0 0 24 24" fill="primary">
              <path d="M12 4l4 4L4 20H0v-4L12 4zm2-2l2-2 4 4-2 2-4-4z" />
            </svg>
          </span>
        </button>
      </div>
      <div v-if="moreCard" class="mt-4">
        ...
        <div v-if="!editing" class="text-sm">{{ target.description }}</div>
        <textarea
          v-else
          v-on:click.stop.prevent
